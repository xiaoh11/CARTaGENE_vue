...
<script setup>
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const response = await axios.get(`${api}/variants/region/snv/clinVargraph/${chrom}-${start}-${stop}`); 
  ...
  const initializeChart = () => {
    ...
    if (props.segmentBounds && props.segmentRegions) {
        x.value = d3.scaleLinear()
          .range(props.segmentBounds)
          .domain(props.segmentRegions);
    } else {
        console.error("props.segmentBounds or props.segmentRegions is not initialized.");
        return;
    }
    // x = d3.scaleLinear()
    //   .domain(props.segmentRegions)
  ...
  const updateChart = () => {
    ...
    const lines = svg.value.selectAll(".lollipop-line")
      .data(!cagOnly.value ? data.value : data.value.filter(d => d.isInMongo === true), d => d.position);
    ...
    circles.join(
      enter => enter.append("circle")
        ...
        .attr("r", d => cagOnly.value ? 10 : (d.isInMongo == true ? 10 :4))
  ...
  const addLegend = () => {
    ...
    legend.selectAll("circle")
      ...
      //     .attr("fill", "url(#multi-color-gradient)")
      //     .style("cursor", "pointer")
      //     .attr("r", 15)
      //   tooltip
      //     .style("opacity", 1)
      //     .html("click to show CaG only / all")
      //     .style("left", (event.clientX - svgRect.left + 10) + "px")
  ...
  // in region page, segmentRegions should be fixed
